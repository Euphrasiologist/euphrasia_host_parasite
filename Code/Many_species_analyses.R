...
ggsave(
  filename = "./Figures/Many_species/quick_visualisation", plot = plot_1.1,
...
write.csv(
  x = aod::wald.test(cov(manysp.2$Sol[, 2:6, drop = F]), colMeans(manysp.2$Sol[, 2:6, drop = F]), Terms = 1:5)$result$chi2,
  file = "./Data/Many_species/Model_outputs/Host_parasite_interaction/Euphrasia_pop_sig.csv"
)


write.csv(
  x = specify_decimal(summary(manysp.2)$solutions, 4),
  file = "./Data/Many_species/Model_outputs/Host_parasite_interaction/Model_solutions.csv"
)

write.csv(
  x = data.table(
    HOST = MCMCReppois(mod = manysp.2, y = "Host_code"),
...
ggsave(
  filename = "./Figures/Many_species/posterior_interaction_modes_mod_1.pdf", plot = plot_2.1,
  device = "pdf", width = 21, height = 6, units = "in"
)

m1 <- data.table(
  Density = (manysp.2$VCV[, c(1)]) / rowSums(manysp.2$VCV),
  Trait = as.factor(rep("Host contribution", 1000))
)
m2 <- data.table(
  Density = (manysp.2$VCV[, c(2)]) / rowSums(manysp.2$VCV),
  Trait = as.factor(rep("Host:Euphrasia interaction", 1000))
)
m3 <- data.table(
  Density = (manysp.2$VCV[, c(3)]) / rowSums(manysp.2$VCV),
  Trait = as.factor(rep("Residual", 1000))
)
final <- rbind(m1, m2, m3)
final$Density <- as.numeric(final$Density)

final$Trait <- factor(x = final$Trait, levels = rev(c(
  "Host:Euphrasia interaction",
  "Host contribution",
  "Residual"
)))
...
(plot_3.1 <- dat %>%
  ...
  scale_y_continuous(
    breaks = c(0, 0.6931472, 1.609438, 2.302585, 3.218876, 4.60517), # 1,2, 5, 10, 50, 100
...
ggsave(
  filename = "./Figures/Many_species/population_cum_nodes.pdf", plot = plot_3.1,
  device = "pdf", width = 10, height = 6, units = "in", useDingbats = FALSE
)
ggsave(
  filename = "./Figures/Many_species/population_cum_nodes.jpeg", plot = plot_3.1,
...
(plot_3.1.1 <- ggplot(dat, aes(x = meanH, y = mean)) +
  ...
  geom_text(
    data = data.frame(mean = rep(5, 4), meanH = rep(0, 4), Euphrasia_sp2 = c(
      "Euphrasia anglica",
      "Euphrasia micrantha",
      "Euphrasia tetraquetra",
      "Euphrasia vigursii"
    )), label = letters[5:8], nudge_x = -0.1,
    nudge_y = 0, size = 8
  ) +
  theme_bw() +
  theme(
  ...
  scale_x_continuous(
    breaks = c(0, 0.6931472, 1.609438, 2.70805),
    labels = round(c(exp(0), exp(0.6931472), exp(1.609438), exp(2.70805)), 0)
  ) +
  scale_fill_manual(
    name = expression(paste(italic("Euphrasia"), " species")),
    limits = c("Euphrasia anglica", "Euphrasia micrantha", "Euphrasia tetraquetra", "Euphrasia vigursii"),
    values = c(cbPalette[2], cbPalette[3], cbPalette[4], cbPalette[5])
  ))
...
ggsave(
  filename = "./Figures/Many_species/population_cum_nodes_mean_tes_bt.pdf", plot = plot_3.1_tes,
  device = "pdf", width = 17, height = 9, units = "in", useDingbats = FALSE
)
ggsave(
  filename = "./Figures/Many_species/population_cum_nodes_mean_tes_bt.jpeg", plot = plot_3.1_tes,
...
plot_3.2 <- rnodes3[, .(
  mean = mean(Reproductive_nodes),
  sem = sd(Reproductive_nodes) / sqrt(.N),
  N = .N
), by = c("Euphrasia_sp2", "Host_code", "Population")][Euphrasia_sp2 %in% c("Euphrasia tetraquetra", "Euphrasia vigursii")] %>%
  ggplot(aes(x = reorder(Host_code, mean), y = mean)) +
...
ggsave(
  filename = "./Figures/Many_species/population_cum_nodes_vig_tet", plot = plot_3.2,
  device = "pdf", width = 10, height = 6, units = "in"
)
...
write.csv(
  x = summary(manysp.3)$solutions,
  file = "./Data/Many_species/Model_outputs/Tet_vs_Vig/Model_Solutions.csv"
)

write.csv(
  x = data.table(
    HOST = MCMCReppois(mod = manysp.3, y = "Host_code"),
...
write.csv(
  x = Solapply(manysp.3)[order(Grouped_Value)][, .(
    Variable = Variable,
...
ggsave(
  filename = "./Figures/Many_species/vig_tet_interation_plot", plot = plot_4.1,
  device = "pdf", width = 10, height = 6, units = "in"
)
...
write.csv(
  x = summary(manysp.4)$solutions,
  file = "./Data/Many_species/Model_outputs/Micrantha/Model_Solutions.csv"
)

write.csv(
  x = specify_decimal(data.table(
    HOST = MCMCReppois(mod = manysp.4, y = "Host_code"),
    HOST_GIVEN_POPULATION = MCMCReppois(mod = manysp.4, y = "Host_given_population")
  ), 6),
...
write.csv(
  x = Solapply(manysp.4)[order(Grouped_Value)][, .(
    Variable = Variable,
    Grouped_Value = exp(Grouped_Value)
  )],
...
# solutions
write.csv(
  x = summary(manysp.5)$solutions,
  file = "./Data/Many_species/Model_outputs/Anglica/Model_Solutions.csv"
)

write.csv(
  x = specify_decimal(data.table(
    HOST = MCMCReppois(mod = manysp.5, y = "Host_code"),
    HOST_GIVEN_POPULATION = MCMCReppois(mod = manysp.5, y = "Host_given_population")
  ), 6),
...
write.csv(
  x = Solapply(manysp.5)[order(Grouped_Value)][, .(
    Variable = Variable,
    Grouped_Value = exp(Grouped_Value)
  )],
...
ggsave(
  filename = "./Figures/Many_species/micrantha_interation", plot = plot_5.1,
  device = "pdf", width = 10, height = 6, units = "in"
)
...
ggsave(
  filename = "./Figures/Many_species/anglica_interation", plot = plot_5.2,
  device = "pdf", width = 10, height = 6, units = "in"
)