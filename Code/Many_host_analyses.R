...
flmeans <- floweringsofar2[, .(
  mean = as.numeric(specify_decimal(mean(Days_since_germination, na.rm = TRUE), 4)),
  sem = specify_decimal(sd(Days_since_germination, na.rm = TRUE) / sqrt(.N), 4)
), by = .(Host_Species)][order(-mean)]
...
write.csv(
  x = specify_decimal(summary(mcmcfix2)$solutions, 4),
  file = "./Data/Many_hosts/Model_outputs/Days_to_flower/Days_solutions.csv"
)

# wald test of annual/perennial
write.csv(
  x = aod::wald.test(cov(mcmcfix2$Sol[, 2, drop = F]), colMeans(mcmcfix2$Sol[, 2, drop = F]), Terms = 1)$result,
...
write.csv(
  x = aod::wald.test(cov(mcmcfix2$Sol[, 3:6, drop = F]), colMeans(mcmcfix2$Sol[, 3:6, drop = F]), Terms = 1:4)$result,
  file = "./Data/Many_hosts/Model_outputs/Days_to_flower/Functional_Group_Wald_Test.csv"
)

# effect of phylogeny
write.csv(list(
  posterior.mode(as.mcmc((mcmcfix2$VCV[, c(2)]) / rowSums(mcmcfix2$VCV))),
...
write.csv(list(
  posterior.mode(as.mcmc(rowSums(mcmcfix2$VCV[, c(1, 2)]) / rowSums(mcmcfix2$VCV))),
...
# units
write.csv(list(
  posterior.mode(as.mcmc((mcmcfix2$VCV[, c(3)]) / rowSums(mcmcfix2$VCV))),
...
write.csv(
  x = daysvcv <- cbind(Response = "Days to flower", VCVapply(mcmcfix2)),
...
survivaldata[Time == 3, .(
  "0" = sum(as.integer(y == 0)),
  "1" = sum(as.integer(y != 0))
), by = "Name"][order(-`1`)]
...
write.csv(
  x = list(
    posterior.mode(as.mcmc(rowSums(eha.1$VCV[, c(1, 2)]) / rowSums(eha.1$VCV))),
...
write.csv(
  x = list(
    posterior.mode(eha.1$VCV[, c(2)] / rowSums(cbind(
      eha.1$VCV[, c(2)],
      eha.1$VCV[, c(1)]
    ))),
    HPDinterval(eha.1$VCV[, c(2)] / rowSums(cbind(
      eha.1$VCV[, c(2)],
      eha.1$VCV[, c(1)]
    )))
  ),
...
write.csv(
  x = ehavcv <- cbind(Response = "Survival", VCVapply(eha.1)),
...
# save the solutions
write.csv(
  x = specify_decimal(summary(mcmcfix3)$solutions, 4),
  file = "./Data/Many_hosts/Model_outputs/Reproductive_nodes_end/Reprod_solutions.csv"
)

# AnnPer
write.csv(
  x = aod::wald.test(cov(mcmcfix3$Sol[, 2, drop = F]), colMeans(mcmcfix3$Sol[, 2, drop = F]), Terms = 1)$result$chi2,
  file = "./Data/Many_hosts/Model_outputs/Reproductive_nodes_end/AnnPer_Wald_Test.csv"
)
# Functional group
write.csv(
  x = aod::wald.test(cov(mcmcfix3$Sol[, 3:6, drop = F]), colMeans(mcmcfix3$Sol[, 3:6, drop = F]), Terms = 1:4)$result$chi2,
  file = "./Data/Many_hosts/Model_outputs/Reproductive_nodes_end/Functional_Group_Wald_Test.csv"
)

# joint phylogenetic distribution
write.csv(list(
  posterior.mode(as.mcmc(rowSums(mcmcfix3$VCV[, c(1, 2)]) / rowSums(mcmcfix3$VCV))),
...
# 74.5%
write.csv(list(
  posterior.mode(mcmcfix3$VCV[, c(2)] / rowSums(cbind(
    mcmcfix3$VCV[, c(2)],
    mcmcfix3$VCV[, c(1)]
  ))),
  # 14.8% - 96.1%
  HPDinterval(mcmcfix3$VCV[, c(2)] / rowSums(cbind(
    mcmcfix3$VCV[, c(2)],
    mcmcfix3$VCV[, c(1)]
  )))
),
...
# variance components
write.csv(
  x = nodesvcv <- cbind(Response = "End of season reproduction", VCVapply(mcmcfix3)),
...
allgrowth5 <- melt.data.table(
  data = allgrowth4, id.vars = c("Unique_ID", "ID_No", "AnnPer", "Functional_group", "CSR", "Family", "Name"),
...
allgrowth7[, .(
  mean = mean(Nodes),
  sem = sd(Nodes) / sqrt(.N)
), by = .(AnnPer, Name, Time)][Time == 4][order(mean)]
...
# summary output
write.csv(
  x = specify_decimal(summary(mcmcfix4)$solutions, 4),
  file = "./Data/Many_hosts/Model_outputs/Reproduction_over_time/OT_Solutions.csv"
)
# functional group
write.csv(
  x = aod::wald.test(cov(mcmcfix4$Sol[, 5:8, drop = F]), colMeans(mcmcfix4$Sol[, 5:8, drop = F]), Terms = 1:4)$result$chi2,
  file = "./Data/Many_hosts/Model_outputs/Reproduction_over_time/Functional_Group_Wald_Test.csv"
)
# life history host: time point
write.csv(
  x = aod::wald.test(cov(mcmcfix4$Sol[, 9:10, drop = F]), colMeans(mcmcfix4$Sol[, 9:10, drop = F]), Terms = 1:2)$result$chi2,
  file = "./Data/Many_hosts/Model_outputs/Reproduction_over_time/AnnPerInt_Wald_Test.csv"
)
# life history of host
write.csv(
  x = aod::wald.test(cov(mcmcfix4$Sol[, 4, drop = F]), colMeans(mcmcfix4$Sol[, 4, drop = F]), Terms = 1)$result$chi2,
...
ggsave(
  filename = "./Figures/Many_hosts/fig_1_tes_labs.pdf", plot = fig_1_tes_labs,
  device = "pdf", width = 10, height = 6, units = "in"
)
ggsave(
  filename = "./Figures/Many_hosts/fig_1_tes_labs.jpeg", plot = fig_1_tes_labs,
  device = "jpeg", width = 10, height = 6, units = "in"
)

ggsave(
  filename = "./Figures/Many_hosts/two_families_survival.pdf", plot = plot_1,
  device = "pdf", width = 7, height = 6, units = "in"
)
ggsave(
  filename = "./Figures/Many_hosts/two_families_survival.jpeg", plot = plot_1,
...
phylodat1.1 <- phylodat[, .(
  Mean.Cum = mean((na.omit(C.R.Nodes.F) + 1)),
...
(finalplot <- facet_plot(p + xlim_tree(1.1),
  panel = "Cumulative Reproductive Nodes",
  data = phylodat1.1,
  mapping = aes(
    x = Mean.Cum,
...
ggsave(
  filename = "./Figures/Many_hosts/Cum_reprod_nodes_phylo.pdf", plot = plot_2,
  device = "pdf", width = 11, height = 12, units = "in", useDingbats = FALSE
)
ggsave(
  filename = "./Figures/Many_hosts/Cum_reprod_nodes_phylo.jpeg", plot = plot_2,
...
ggsave(
  filename = "./Figures/Many_hosts/ann_per_time.pdf", plot = plot_3,
...
ggsave(
  filename = "./Figures/Many_hosts/average_host.pdf", plot = plot_4,
  device = "pdf", width = 13.5, height = 7.5, units = "in"
)
ggsave(
  filename = "./Figures/Many_hosts/average_host.jpeg", plot = plot_4,
...
ggsave(
  filename = "./Figures/Many_hosts/joint_variance_explained.pdf", plot = plot_5,
